...
export function TodaysTrades({ trades }: { trades: Trade[] }) {
  ...
  const total = trades.reduce((s, t) => s + (displayPnl(t.pnl, t.fees) ?? 0), 0);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 font-mono text-sm text-zinc-400">
            ...
            <span className={total >= 0 ? "text-primary" : "text-destructive"}>{formatMoney(total)}</span>
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="overflow-hidden rounded-xl border border-white/[0.06]">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/[0.04]">
            {trades.map((t) => {
              const pnl = displayPnl(t.pnl, t.fees);
              return (
                <tr key={t.id} className="hover:bg-white/[0.02]">
                  <td className="px-4 py-3">
                    <Link href={`/trades/${t.id}`} className="font-mono font-medium text-white hover:text-primary">
                      {t.symbol}
                    </Link>
                  ...
                  <td
                    className={`px-4 py-3 text-right font-mono ${
                      (pnl ?? 0) > 0 ? "text-primary" : (pnl ?? 0) < 0 ? "text-destructive" : "text-zinc-500"
                    }`}
                  >
